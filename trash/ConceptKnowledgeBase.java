...
import thinker.mind.memory.IKnowledgeBase;
...
public class ConceptKnowledgeBase implements IKnowledgeBase {

	private IModifiableRelationGraph<IConceptNode, IConceptRelationType> conceptGraph;
	...
	public ConceptKnowledgeBase(IConcept self) {
		conceptGraph = new RelationGraph<>(Set.of(RelationProperties.STORAGE_TYPE, RelationProperties.TRUTH_TYPE,
				RelationProperties.CONFIDENCE, RelationProperties.INFO_SOURCE));
		immutableView = new ImmutableGraphView<>(conceptGraph);
		this.self = self;
	...
	public IKnowledgeBase knowsConceptCheckParent(IConcept concept) {
		...
		for (IKnowledgeBase parent : this.parents) {
			IKnowledgeBase par = parent.knowsConceptCheckParent(concept);
			if (par != null)
				return par;
		}
		return null;
	...
	public IKnowledgeBase hasRelationCheckParent(IConcept from, IConceptRelationType type, IConcept to) {
		if (this.hasRelation(from, type, to))
			return this;
		for (IKnowledgeBase parent : this.parents) {
			IKnowledgeBase par = parent.hasRelationCheckParent(from, type, to);
			if (par != null)
				return par;
		}
		return null;
	...
	public IKnowledgeBase hasRelationCheckParent(IConcept from, IConcept to) {
		if (this.hasRelation(from, to))
			return this;
		for (IKnowledgeBase parent : this.parents) {
			IKnowledgeBase par = parent.hasRelationCheckParent(from, to);
			if (par != null)
				return par;
		}
		return null;
	...
	public IMultiKnowledgeBaseIterator<IConceptRelationType> getRelationTypesFromCheckParent(IConcept from) {
		return new ConceptStorageParentIterator<IConceptRelationType>(
				() -> conceptGraph.getConnectingEdgeTypes(new ConceptNode(from)).iterator(),
				(parent) -> parent.getRelationTypesFromCheckParent(from));
	}
	...
	public IMultiKnowledgeBaseIterator<? extends IConcept> getConnectedConceptsCheckParent(IConcept from) {
		return new ConceptStorageParentIterator<>(() -> conceptGraph.getNeighbors(new ConceptNode(from)).stream()
				.map(IConceptNode::getConcept).iterator(), (parent) -> parent.getConnectedConceptsCheckParent(from));
	}
	...
	public IMultiKnowledgeBaseIterator<? extends IConcept> getConnectedConceptsCheckParent(IConcept from,
		...
		return new ConceptStorageParentIterator<>(() -> conceptGraph.getNeighbors(new ConceptNode(from), type).stream()
				...
				(parent) -> parent.getConnectedConceptsCheckParent(from, type));
	}
	...
	public Map<IKnowledgeBase, TruthType> getTruthTypeOfRelationCheckParents(IConcept from, IConceptRelationType type,
		...
		for (IKnowledgeBase para : parents) {
			types.putAll(para.getTruthTypeOfRelationCheckParents(from, type, to));
		}
		return types;
	}
	...
	public Map<IKnowledgeBase, Float> getSocialBondValueCheckParents(IConcept from, ISocialBondTrait trait,
		...
		for (IKnowledgeBase para : parents) {
			types.putAll(para.getSocialBondValueCheckParents(from, trait, to));
		}
		return types;
	}
	...
	public ConceptKnowledgeBase clone() {
		...
		base.immutableView = new ImmutableGraphView<>(base.conceptGraph);
