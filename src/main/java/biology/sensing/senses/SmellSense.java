...
public class SmellSense extends AbstractSense {

	public SmellSense(String name) {
	...
	public Map<? extends IComponentPart, ? extends Collection<? extends IMaterialLayer>> getSensableParts(
		...
		for (IComponentPart part : forEntity.getPhysical().getParts()) {
			// TODO material penetration
			for (IMaterialLayer materi : part.getMaterials().values()) {
				if (materi.getState().gone())
					continue;
				if (materi.getProperty(SenseProperty.SMELL, false).canBeSensed())
					map.put(part, materi);
